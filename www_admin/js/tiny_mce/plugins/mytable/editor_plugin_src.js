...
(function() {
	...
	tinymce.create('tinymce.plugins.MyTablePlugin', {
		...
		execCommand : function(cmd, ui, val) {
			...
			switch (cmd) {
				...
				case "mceTableDelete":
					...
					// stripe reload
					if (cmd == "mceTableInsertRowBefore" || cmd == "mceTableInsertRowAfter" || cmd == "mceTableCutRow" ||
						cmd == "mceTablePasteRowBefore" || cmd == "mceTablePasteRowAfter" || cmd == "mceTableDeleteRow") {
						var focusElm = ed.selection.getNode();
						var tableElm = ed.dom.getParent(focusElm, "table");
						// search striping
						if ($(tableElm).find("tr").hasClass("table-odd")) {
							$(tableElm).find("tr").removeClass("table-odd")
							this._doExecCommand(cmd, ui, val);
							$(tableElm).find("tr:nth-child(odd)").addClass("table-odd");
						} else {
							this._doExecCommand(cmd, ui, val);
						}						
					} else {
	...
	// Register plugin
	tinymce.PluginManager.add('mytable', tinymce.plugins.MyTablePlugin);
})();